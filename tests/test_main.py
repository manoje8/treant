...
async def test_process_document_pdf(mock_parser, tmp_path):
    ...
    with patch("main.get_parser_method", return_value=mock_parser):
        result = await process_document(pdf_file, ParseMethod.DOCLING)
...
async def test_process_document_html(mock_parser, tmp_path):
    ...
    with patch("main.get_parser_method", return_value=mock_parser):
        result = await process_document(html_file, ParseMethod.DOCLING)
...
async def test_process_document_office(mock_parser, tmp_path):
    ...
    with patch("main.get_parser_method", return_value=mock_parser):
        result = await process_document(doc_file, ParseMethod.DOCLING)
...
async def test_process_document_file_too_large(mock_parser, tmp_path):
    ...
    with patch("main.settings.MAX_UPLOAD_BYTES", 1):
...
async def test_save_content(tmp_path):
    ...
    await _save_content(content, output_file)
...
def test_display_content_stats(caplog):
    ...
    _display_content_stats(content)
